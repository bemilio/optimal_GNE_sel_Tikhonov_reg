...
if create_new_dataframe_file or not os.path.exists('saved_dataframe.pkl'):
    ...
    if load_files_from_current_dir:
        ...
        dummy = False
        for filename in os.listdir(directory):
            if filename.find('.pkl')>=0 and not dummy:
    ...
    x_1 = range(1, Steps_between_iterations * (N_iterations-9*N_iterations//10), Steps_between_iterations)
    ...
    ax[0,0].plot(x_1, torch.mean(residual_store_hsdm[:, index_hsdm_to_plot, :(N_iterations-9*N_iterations//10)], dim=0), color='g', label="HSDM", linestyle=next(linecycler))
    ...
    ax[0,0].plot(x_1, torch.mean(residual_store_tich[:, index_tich_to_plot, :(N_iterations-9*N_iterations//10)], dim=0), color='m', label="Alg. 1", linestyle=next(linecycler))
    ...
    ax[0,0].set_xlim(1, (N_iterations-N_iterations/10) * Steps_between_iterations)
    ...
    ax[1,0].plot(x_1, torch.mean(relative_advantage_hsdm, dim=0)[:(N_iterations-9*N_iterations//10)] , color='g', label="HSDM",linestyle=next(linecycler))
    ...
    ax[1,0].plot(x_1, torch.mean(relative_advantage_tich, dim=0)[:(N_iterations-9*N_iterations//10)], color='m', label="Alg. 1", linestyle=next(linecycler))
    ...
    ax[1,0].set_xlim(1, (N_iterations-9*N_iterations//10) * Steps_between_iterations)
    ...
    ax[0,1].set_ylim(10**(-3), 10**(-2))
    ax[0,1].set_yticks(np.logspace(-3,-2, 2))
    ax[0, 1].set_yticklabels(np.logspace(-3, -2, 2), minor=True)
    ...
    xy_1_a = ((9 * N_iterations // 10)+2, torch.mean(residual_store_hsdm[:, index_hsdm_to_plot,(N_iterations - 9 * N_iterations // 10)], dim=0))
    xy_1_b = ((9 * N_iterations // 10) + 2,
              torch.mean(residual_store_tich[:, index_tich_to_plot, (N_iterations - 9 * N_iterations // 10)], dim=0))
    xy_2_a = ((9 * N_iterations // 10)+2, ylims_res[0])
    xy_2_b = ((9 * N_iterations // 10) +2, ylims_res[1])
    ...
    xy_1_a = ((9 * N_iterations // 10)+2, relative_advantage_hsdm[0, (N_iterations - 9 * N_iterations // 10)])
    xy_1_b = ((9 * N_iterations // 10) + 2, relative_advantage_tich[0,(N_iterations - 9 * N_iterations // 10)])
    xy_2_a = ((9 * N_iterations // 10)+2, ylims_res[0])
    xy_2_b = ((9 * N_iterations // 10) +2, ylims_res[1])
    ...
    for i in all_indexes_with_approx_reg_to_plot_and_alpha_to_plot:
        ax[0,0].plot(x_1, torch.mean(residual_store_tich[:, i, :(N_iterations-9*N_iterations//10)], dim=0), label=str(parameters_tested_tich[i][0]), linestyle=next(linecycler))
    ...
    for i in all_indexes_with_approx_reg_to_plot_and_alpha_to_plot:
        ...
        ax[1,0].plot(x_1, torch.mean(values_to_plot, dim=0)[:(N_iterations-9*N_iterations//10)], label=str(parameters_tested_tich[i][0]), linestyle=next(linecycler))
    ...
    ylims_res = ax[1,1].get_ylim()
    xy_1_a = ((9 * N_iterations // 10)+2, relative_advantage_tich[0, (N_iterations - 9 * N_iterations // 10)])
    xy_1_b = ((9 * N_iterations // 10) + 2, relative_advantage_tich[0,(N_iterations - 9 * N_iterations // 10)])
    xy_2_a = ((9 * N_iterations // 10)+2, ylims_res[0])
    xy_2_b = ((9 * N_iterations // 10) +2, ylims_res[1])
    con_a = ConnectionPatch(xyA=xy_1_a, xyB=xy_2_a, coordsA="data", coordsB="data", axesA=ax[1][0], axesB=ax[1][1], color="k")
